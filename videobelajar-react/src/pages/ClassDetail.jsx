...
const ClassDetail = () => {
    ...
    const getIcon = (type, status) => {
        const isCompleted = status === 'completed' || course.progress === 100 || (activeItem.id === activeItemId && isPassed);
    ...
    const getItemStatus = (item) => {
        if (item.id === activeItemId && isPassed) return 'completed';
    ...
    const footerConfig = useMemo(() => {
        if (quizStarted) {
            return {
                onPrev: null, onNext: null, customRightButton: null,
                middleContent: (
                    <div className="w-full text-center font-bold text-white text-sm md:text-lg truncate px-4">
                        {course.title}
                    </div>
                )
            };
    ...
    }, [quizStarted, currentQuestionIndex, currentQuestions, prevItem, nextItem, course.progress, course.title, currentIndex, allItems.length]);
    ...
    return (
        <div className="min-h-screen bg-white font-poppins flex flex-col relative">
            ...
            <Modal isOpen={showResultModal} onClose={() => setShowResultModal(false)}>
                <div className="text-center flex flex-col items-center">
                    <div className={`w-20 h-20 mx-auto rounded-full flex items-center justify-center mb-4 ${isPassed ? 'bg-green-100' : 'bg-red-100'}`}><i className={`fa-solid ${isPassed ? 'fa-trophy text-green-500' : 'fa-xmark text-red-500'} text-4xl`}></i></div>
                    <h3 className="text-2xl font-bold text-gray-900 mb-2">{isPassed ? 'Selamat! Anda Lulus' : 'Yah, Belum Lulus'}</h3>
                    <p className="text-gray-500 mb-6">Nilai kamu: <span className={`font-bold text-xl ${isPassed ? 'text-green-600' : 'text-red-500'}`}>{score}</span> / 100</p>
                    <div className="flex flex-col gap-3 w-full">
                        {isPassed ? (
                            <button onClick={() => { setShowResultModal(false); if(nextItem) handleItemClick(nextItem); }} className="w-full bg-[#3ECF4C] text-white font-bold py-3 rounded-xl hover:bg-green-600 transition-colors">Lanjut Materi Berikutnya</button>
                        ) : (
                            <button onClick={handleRetryQuiz} className="w-full bg-[#FF5722] text-white font-bold py-3 rounded-xl hover:bg-orange-600 transition-colors">Ulangi Ujian</button>
                        )}
                        <button onClick={() => setShowResultModal(false)} className="text-gray-500 text-sm font-semibold hover:text-gray-800 mt-2">Tutup</button>
                    </div>
                </div>
            </Modal>
            ...
            <div className="flex-1 max-w-[1440px] mx-auto w-full flex flex-col lg:flex-row pb-[80px]">
                ...
                <div className="flex-1 w-full lg:w-[70%] bg-white min-h-0 border-r border-gray-100 flex flex-col">
                            ...
                            {!quizStarted ? (
                                ...
                                <div className="flex flex-col md:flex-row h-full">
                                    ...
                                    <div className="flex-1 p-5 md:p-10 flex flex-col bg-white">
                                        ...
                                        <p className="text-gray-600 leading-relaxed mb-6 text-sm md:text-base">{currentQuestions[currentQuestionIndex].question}</p>
                    ...
                    {/* MOBILE MODUL LIST (Dengan Review Button di bawahnya) */}
                    <div className="lg:hidden bg-white p-5 pt-6 pb-20 border-t border-gray-100">
                        ...
                        {/* REVIEW BUTTON (MOBILE) - Ditambahkan di sini agar bisa di-scroll */}
                        <div className="mt-6 mb-4">
                            <button className="w-full bg-[#FFC107] hover:bg-yellow-500 text-white font-bold py-3 rounded-xl shadow-sm flex items-center justify-center gap-2">
                ...
                <div className="hidden lg:flex w-[30%] flex-col sticky top-[80px] h-[calc(100vh-80px)] bg-white border-l border-gray-100">
                    ...
                    {/* REVIEW BUTTON (DESKTOP) - Ditambahkan Kembali */}
                    <div className="p-4 border-t border-gray-100 bg-white sticky bottom-0 z-10 shadow-[0_-5px_15px_rgba(0,0,0,0.03)]">
                        <button className="w-full bg-[#FFC107] hover:bg-yellow-500 text-white font-bold py-3 rounded-xl shadow-sm transition-all transform hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2">
                            <i className="fa-regular fa-star"></i> Beri Review & Rating
                        </button>
                    </div>
                </div>
